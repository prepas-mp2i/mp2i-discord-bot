...
class GuildWrapper:
    ...
    def _fetch(self) -> Optional[GM]:
        ...
        """
        try:
            return db.execute(select(GM).where(GM.id == self.guild.id)).scalar_one()
        except sqlalchemy.exc.NoResultFound:
            return None

    def register(self) -> None:
        db.execute(insert(GM).values(id=self.guild.id, name=self.guild.name))
    ...
    def update(self, **kwargs) -> None:
        ...
        """
        db.execute(update(GM).where(GM.id == self.guild.id).values(**kwargs))
        self.__model = self._fetch()
